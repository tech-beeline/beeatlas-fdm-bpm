...
import ru.beeline.fdmbpm.dto.CriteriaExcelDTO;
...
public class ImportProcessService {
    ...
    public Integer uploadingDataFromExcel(String entityType, Boolean sync, Integer docId) {
        ...
        if (response.getStatusCode().is2xxSuccessful()) {
            log.info("Документ успешно извлечен со статусом: {}", response.getStatusCode());
            byte[] fileData = response.getBody();
            String fileName = response.getHeaders().getContentDisposition().getFilename();
            Set<String> expectedColumns = new HashSet<>();
            if (entityType.equals("capability_criterias")) {
                expectedColumns = new HashSet<>(Arrays.asList("type", "code", "criterionname", "value",
                        "grade", "comment"));
            } else {
                expectedColumns = new HashSet<>(Arrays.asList("code", "name", "description", "parents",
            ...
            try {
                ...
                if (fileValidation(fileName) && columnValidation(tempFile, expectedColumns)) {
                    log.info("Файл и столбцы валидны.");
                    if (entityType.equals("capability_criterias")) {
                        List<CriteriaExcelDTO> criteriaExcelDTOS;
                        try {
                            criteriaExcelDTOS = convertCriteriaExcelDTO(tempFile);
                        } catch (Exception e) {
                            log.error("Ошибка парсинга файла: {}", fileName);
                        ...
                        fileData = null;
                        System.gc();
                        return registerAndSendCriteriaPackage(operation, criteriaExcelDTOS, docId);
                    } else {
                    ...
                    log.error("Недопустимый формат файла: {}", fileName);
                    PackageRegistrationResponseDTO responseDTO = packageClient.registerPackage(operation,
                            1, "excel", "VALIDATE ERROR", docId);
                    if (responseDTO == null) {
                        throw new ValidationException("Not valid file, not registered");
    ...
    private Integer registerAndSendCriteriaPackage(String operation, List<CriteriaExcelDTO> criteriaExcelDTOS,
                                                   Integer docId) {
        try {
            log.info("Register package, operation: {} , excelCriteriaDTOS size: {}", operation, criteriaExcelDTOS.size());
            PackageRegistrationResponseDTO responseDTO = packageClient.registerPackage(operation, criteriaExcelDTOS.size(),
                    "excel", docId);
            log.info("packageId: {}", responseDTO.getPackageId());
            ObjectNode messagePayload = createMessagePayloadForCriteria(responseDTO, criteriaExcelDTOS);
            log.info("Send to package-queue");
            rabbitService.sendMessage(packageQueueName, objectMapper.writeValueAsString(messagePayload));
            log.info("method completed");
            return responseDTO.getPackageId();
    ...
    private String getOperationType(String entityType) {
        ...
        } else if (entityType.equals("capability_criterias")) {
            return "UPDATE_CRITERIA_FROM_EXCEL";
        } else {
    ...
    private ObjectNode createMessagePayloadForCriteria(PackageRegistrationResponseDTO responseDTO, List<CriteriaExcelDTO> criteriaExcelDTOS) {
        ObjectNode messagePayload = objectMapper.createObjectNode();
        messagePayload.put("packageId", responseDTO.getPackageId());
        ArrayNode payloadArray = messagePayload.putArray("payload");
        criteriaExcelDTOS.forEach(criteriaDTO -> {
            ObjectNode item = objectMapper.createObjectNode();
            item.put("code", criteriaDTO.getCode());
            item.put("type", criteriaDTO.getType());
            item.put("criterionName", criteriaDTO.getCriterionName());
            item.put("value", criteriaDTO.getValue());
            item.put("grade", criteriaDTO.getGrade());
            item.put("comment", criteriaDTO.getComment());
            payloadArray.add(item);
        });
        return messagePayload;
    }
    ...
    private Boolean columnValidation(File tempFile, Set<String> expectedColumns) {
        try (Workbook workbook = WorkbookFactory.create(tempFile)) {
            ...
            log.error("Ошибка проверки имен колонок в файле : ", e);
            return false;
        }
    }
    ...
    public List<CriteriaExcelDTO> convertCriteriaExcelDTO(File tempFile) {
        List<CriteriaExcelDTO> excelDTOList = new ArrayList<>();
        try (InputStream inputStream = new FileInputStream(tempFile);
             Workbook workbook = new XSSFWorkbook(inputStream)) {
            Sheet sheet = workbook.getSheetAt(0);
            Row headerRow = sheet.getRow(0);
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                CriteriaExcelDTO dto = new CriteriaExcelDTO();
                for (int j = 0; j < headerRow.getLastCellNum(); j++) {
                    Cell headerCell = headerRow.getCell(j);
                    Cell cell = row.getCell(j);
                    String columnName = normalizeColumnName(headerCell.getStringCellValue());
                    ...
                    switch (columnName) {
                        case "type":
                            dto.setType(cellValue);
                            break;
                        case "code":
                            dto.setCode(cellValue);
                            break;
                        case "criterionname":
                            dto.setCriterionName(cellValue);
                            break;
                        case "value":
                            dto.setValue(cellValue);
                            break;
                        case "grade":
                            dto.setGrade(cellValue);
                            break;
                        case "comment":
                ...
                if (isBlank(dto.getType()) || isBlank(dto.getCode()) ||
                        isBlank(dto.getCriterionName()) || isBlank(dto.getValue())) {
                    log.error("Ошибка в строке " + i + ": обязательные поля не могут быть пустыми");
